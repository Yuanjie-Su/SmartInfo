...
async def fetch_news(
    ...
    crawler_result = None
    logger.debug(f"使用Playwright爬虫开始爬取: {url}")
    try:
        async with PlaywrightCrawler() as crawler:
            crawler_result = await crawler.fetch_single(url)
            logger.debug(f"Playwright爬虫爬取完成: {url}")
    except Exception as e:
        logger.error(f"Playwright爬虫爬取失败: {url}, 错误: {str(e)}", exc_info=True)
        raise ValueError(f"Failed to crawl {url}: {str(e)}")

    if not crawler_result or crawler_result.get("error"):
        error_msg = (
            crawler_result.get("error", "未知错误") if crawler_result else "空结果"
        )
    ...
    logger.debug(f"开始清理HTML并转换为Markdown: {url}")
    ...
    logger.debug(f"Markdown Token计数: {url}, 共{token_size}个token")
    ...
    if token_size > llm_pool._max_input_tokens:
        ...
        try:
            ...
            logger.debug(f"开始执行内容分块: {url}")
            markdown_chunks = get_chunks(cleaned_markdown, num_chunks)
            logger.debug(f"内容分块完成: {url}, 实际生成了{len(markdown_chunks)}个块")
        except Exception as e:
    ...
    logger.debug(f"准备处理{chunk_count}个内容块: {url}")
    ...
    for i, chunk_content in enumerate(markdown_chunks):
        ...
        logger.debug(
            f"开始处理内容块 {i+1}/{chunk_count}: {url}, 块大小: {len(chunk_content)}字节"
        )
        ...
        logger.debug(f"在块 {i+1}/{chunk_count} 中找到 {found_count} 个有效链接: {url}")
    ...
    logger.info(
        f"完成处理URL: {url}, 耗时: {elapsed_time:.2f}秒, 处理了 {summary_count} 篇文章"
    )
...
def _clean_and_prepare_markdown(
    ...
    try:
        ...
        logger.error(f"HTML清理/格式化过程中发生错误: {url}: {e}", exc_info=True)
        return None
...
async def _extract_and_crawl_links(
    ...
    logger.debug(
        f"开始从Markdown中提取链接: {base_url}, Markdown长度: {len(markdown_content)}字节"
    )
    ...
    try:
        ...
        logger.debug(
            f"LLM返回的链接原始结果, 长度: {len(links_str) if links_str else 0}字节"
        )

        # 如果LLM没有返回链接，跳过而不报错
        if not links_str or not links_str.strip() or links_str.strip() == "no":
            logger.warning(f"LLM未发现链接: {base_url}, 跳过链接提取和爬取")
        ...
        if not extracted_links:
            logger.warning(f"未找到有效链接: {base_url}, 跳过链接提取和爬取")
            return sub_original_content_metadata_dict

        logger.debug(f"找到 {len(extracted_links)} 个有效链接, 开始爬取: {base_url}")
        ...
        async with AiohttpCrawler() as sub_crawler:
            ...
            async for crawl_result in sub_crawler.process_urls(
                ...
                processed_count += 1
                logger.debug(
                    f"爬取进度: {processed_count}/{total_links} ({processed_count*100/total_links:.1f}%)"
                )
                ...
                logger.debug(
                    f"成功提取文章: {sub_url}, 标题: {title}, 内容长度: {content_length}字节"
                )
                sub_original_content_metadata_dict[sub_url] = structure_data

        logger.debug(
            f"子链接爬取完成: {base_url}, 成功爬取 {len(sub_original_content_metadata_dict)}/{total_links} 个子链接"
        )
...
async def summarize_content(
    ...
    logger.debug(f"内容分析提示词Token数量: {prompt_tokens}")
    ...
    try:
        ...
        if prompt_tokens > llm_pool._max_input_tokens:
            ...
            if total < num_prompt_chunks:
                logger.error(
                    f"Token数量超限，但文章数量 ({total}) 少于分块数量 ({num_prompt_chunks}), 无法有效分块"
                )
                raise ValueError(
                    f"Token limit exceeded, not enough content to chunk for {url}."
                )

            logger.debug(f"每个块将包含约 {total // num_prompt_chunks} 篇文章")
            ...
            for i in range(num_prompt_chunks):
                ...
                if part_keys:
                    ...
                    logger.debug(
                        f"块 {i+1} 包含 {len(part_keys)} 篇文章, 从索引 {start} 到 {end-1}"
                    )
            ...
            logger.debug(f"已创建 {len(prompt_chunks)} 个提示词块, 准备分批处理: {url}")
            ...
            for i, p_chunk in enumerate(prompt_chunks):
                ...
                try:
                    ...
                    logger.debug(f"成功从块 {i+1} 解析出 {result_count} 篇文章摘要")
                    ...
                    logger.error(
                        f"块 {i+1} 处理过程中发生错误: {url}: {e}", exc_info=True
                    )
            ...
            try:
                ...
                logger.debug(f"成功解析出 {result_count} 篇文章摘要")
    ...
    logger.debug(f"开始处理最终结果，当前有 {len(analysis_result)} 篇文章摘要")
    filtered_result = []
    missing_url_count = 0
    ...
    for result_item in analysis_result:
        ...
        if url_key:
            ...
            logger.debug(
                f"添加文章到最终结果: {url_key}, 标题: {title}, 摘要长度: {summary_length}字节"
            )
    ...
    logger.info(
        f"最终结果处理完成: {url}, 共 {len(filtered_result)} 篇有效文章, 忽略了 {missing_url_count} 篇无URL文章"
    )
