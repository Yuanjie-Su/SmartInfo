...
DEFAULT_EXCLUDE_SELECTORS = [
    # --- 广告与推广内容 ---
    ".ad", ".ads", ".advert", ".advertisement", ".sponsored", ".promo",

    # --- 导航与菜单 ---
    ".menu", ".nav", ".navigation", ".navbar", ".breadcrumbs",
    ...
    # --- 侧边栏（通常非核心内容）---
    ".sidebar", ".widget", ".secondary",
    "#sidebar", "#secondary",

    # --- 社交媒体与分享功能 ---
    ".share", ".social", ".share-bar", ".social-links", ".follow", ".unfollow",

    # --- 用户评论与互动区域 ---
    ".comments", ".comment-respond", ".reply", "#comments", "#respond",

    # --- 元数据与辅助信息 ---
    ".meta", ".post-meta", ".entry-meta", ".byline", ".timestamp",
    ".back-to-top", ".skip-link", ".conditions", ".terms", ".privacy",
    ".disclaimer", ".copyright",

    # --- 推荐与相关内容区域 ---
    ".related", ".related-articles", ".related-posts", ".recommended",
    ".suggestions", ".top-stories", ".trending", ".popular-posts",

    # --- 表单及用户操作类元素 ---
    ".button", ".btn", ".submit", ".search-form", ".subscribe", ".newsletter",
    ".signup", ".join-community", ".contribute", ".report", ".write-article",

    # --- 弹窗、浮层及通知 ---
    ".popup", ".modal", ".overlay", ".cookie-notice", ".cookie-banner", ".gdpr-consent",

    # --- 隐藏及辅助性元素 ---
    ".hidden", "[hidden]", ".screen-reader-text", 

    # --- 杂项、分页、图库（视情况保留或排除） ---
    ".pagination", ".gallery", ".author-box", ".print-link", ".edit-link",
...
def clean_html(
    ...
    # --- Initial Cleaning ---
    # Combine tags and selectors for efficient removal if possible,
    # though doing it separately is safer for complex cases / bs4 versions.
    elements_to_remove = []
    if exclude_tags:
        for tag_name in exclude_tags:
            try:
                # Use find_all which handles tag names
    ...
    if exclude_selectors:
        for selector in exclude_selectors:
            try:
                 # Use select which handles CSS selectors
                elements_to_remove.extend(soup.select(selector))
            except Exception as e:
                logger.warning(f"Error processing exclude_selector '{selector}': {e}")
    
    removed_count = 0
    # Remove elements uniquely and efficiently
    unique_elements = set(elements_to_remove)
    for element in unique_elements:
         # Check if element is still in the tree (might have been removed as part of a parent)
        if element.parent is not None:
            element.decompose()
            removed_count += 1
    logger.debug(f"Removed {removed_count} elements based on exclusions for {base_url}.")
...
def format_html(
    ...
    """
    将清理后的BeautifulSoup对象格式化为指定格式的文本。
    
    Args:
        soup: 已清理的BeautifulSoup对象
        base_url: 网页的基础URL（用于日志记录）
    ...
    try:
        ...
        if output_format == 'markdown':
            # Default options for markdownify can be passed via **kwargs
            if markdownify_options is None:
                markdownify_options = {}
            formatted_content = markdownify(str(target_element), **markdownify_options).strip()
        ...
        logger.error(f"Error during final formatting ({output_format}) for {base_url}: {e}")
        if soup:
             try:
                formatted_content = soup.get_text(separator='\n', strip=True) # Fallback
             except Exception:
                 formatted_content = "" # Final fallback
...
def clean_and_format_html(
    ...
    soup = clean_html(html_content, base_url, exclude_tags, exclude_selectors)
    return format_html(soup, base_url, output_format, markdownify_options)
