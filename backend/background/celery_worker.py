...
#         _process_db_manager = await init_db_connection(db_connection_mode="single")
#         logger.info(f"[PID:{pid}] Database connection initialized.")

#         # 2. Initialize Repositories (use the process's connection implicitly)
#         _process_news_repo = NewsRepository(_process_db_manager._db_resource)
#         _process_source_repo = NewsSourceRepository(_process_db_manager._db_resource)
#         _process_api_key_repo = ApiKeyRepository(_process_db_manager._db_resource)
...
#             logger.debug(f"[PID:{pid}] Using existing event loop for setup.")
#         except RuntimeError:
#             # If no running loop, create a new one
#             loop = asyncio.new_event_loop()
#             asyncio.set_event_loop(loop)
#             logger.debug(f"[PID:{pid}] No running event loop, created new one.")

#         # Run the async setup function with the event loop
#         loop.run_until_complete(_setup_process_dependencies_async())
#     except Exception as e:
#         logger.critical(
