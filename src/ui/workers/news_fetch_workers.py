...
class AsyncWorkerBase(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self._main_task = self.loop.create_task(
                self._main_worker_coroutine(),
                name=f"{worker_name}_main"
            )
            ...
            logger.error(
                f"{worker_name} ({thread_id}) event loop error: {e}", 
                exc_info=True
            )
        finally:
            # Clean up resources
    ...
    async def _main_worker_coroutine(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement _main_worker_coroutine"
        )
        ...
        if self.loop:
            try:
                ...
                if hasattr(self.loop, "shutdown_asyncgens"):
                    logger.debug(f"{worker_name} ({thread_id}): Shutting down async generators...")
    ...
    def cancel_specific_tasks(self, urls: List[str]) -> None:
        ...
        tasks_to_cancel_and_remove: List[Tuple[str, Union[asyncio.Task, asyncio.Future]]] = []
        with self._tasks_lock:
            for url in urls:
                ...
                if task and not task.done():
                    ...
                    logger.debug(f"{worker_name}: Task for {url} already done, marking for removal from tracking.")
                    # If task is already done but still being tracked, also remove it
                    tasks_to_cancel_and_remove.append((url, task))
        
        if tasks_to_cancel_and_remove:
            logger.info(f"{worker_name}: Scheduling cancellation and removal for {len(tasks_to_cancel_and_remove)} specific tasks")
            # Schedule actual cancellation and removal on the event loop thread
            self.loop.call_soon_threadsafe(self._cancel_and_remove_tasks_from_loop, tasks_to_cancel_and_remove)
        else:
            logger.info(f"{worker_name}: No active, non-done tasks found matching the URLs to cancel/remove.")
    
    def _cancel_and_remove_tasks_from_loop(self, tasks_to_process: List[Tuple[str, Union[asyncio.Task, asyncio.Future]]]):
        """Internal helper. Cancels and removes tasks. Must be called from the worker's event loop thread."""
        worker_name = self.__class__.__name__
        if not self.loop or not self.loop.is_running():
            logger.warning(f"{worker_name}: Loop not running during _cancel_and_remove_tasks_from_loop.")
            return
            
        logger.debug(f"{worker_name}: Executing cancel/remove for {len(tasks_to_process)} tasks on loop thread.")
        removed_count = 0
        processed_urls = set() # Keep track of URLs processed in this call

        for url, task in tasks_to_process:
            processed_urls.add(url) # Mark URL as processed for this cancellation request
            ...
            with self._tasks_lock:
                ...
                if url in self._active_tasks and self._active_tasks[url] is task:
                    ...
                    logger.debug(f"{worker_name}: Task for URL {url} was not found in active tracking for removal (might have finished/been removed already).")

        # --- Clear cancellation flags for processed URLs ---  
        if processed_urls:
            with self._specific_cancel_lock:
                initial_cancel_count = len(self._urls_to_cancel)
                self._urls_to_cancel.difference_update(processed_urls) # Remove processed URLs
                cleared_cancel_count = initial_cancel_count - len(self._urls_to_cancel)
                if cleared_cancel_count > 0:
                     logger.debug(f"{worker_name}: Cleared {cleared_cancel_count} specific cancellation flags.")

        if removed_count > 0:
            logger.info(f"{worker_name}: Finished cancellation/removal on loop thread. Removed {removed_count} tasks from tracking.")
        else:
            logger.debug(f"{worker_name}: Finished cancellation/removal on loop thread. No tasks removed from tracking (likely already finished/removed).")
    ...
    def stop(self) -> None:
        ...
        if self.loop and self.loop.is_running():
            ...
            if tasks_to_cancel:
                logger.info(f"Cancelling {len(tasks_to_cancel)} tasks during stop for {worker_name}.")
                for task in tasks_to_cancel:
                    if not task.done():
            ...
            logger.warning(f"{worker_name}: Loop not running or already stopped during stop request.")
...
class CrawlerWorker(AsyncWorkerBase):
    ...
    def __init__(self, urls_with_info: List[Dict[str, Any]], worker_signals: WorkerSignals, parent=None):
        """
    ...
    async def _crawl_single_url(self, url: str) -> None:
        ...
        try:
            ...
            # Perform the crawl
            result = await self._crawler._fetch_single(url, scroll_page=True)
...
class ProcessorWorker(AsyncWorkerBase):
    ...
    def __init__(self, news_service: NewsService, worker_signals: WorkerSignals, 
                llm_base_url: str, llm_api_key: str, parent=None):
        """
    ...
    async def _process_task(self, url: str, html_content: str, source_info: Dict[str, Any]):
        """
