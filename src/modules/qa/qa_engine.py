...
class QAEngine:
    ...
    def __init__(self, sqlite_path: str, chroma_path: str, api_key: str = None, 
        ...
        try:
            ...
            self.collection = self.chroma_client.get_or_create_collection("news_collection")
    ...
    def answer_question(self, question: str, use_history: bool = False, 
        ...
        try:
            ...
            if not question or not question.strip():
                return {"answer": "请输入有效的问题", "sources": [], "error": "问题为空"}
                ...
                for i in range(len(search_results["ids"][0])):
                    ...
                    if distance < 0.7:  # 距离越小，相似度越高
                        ...
                        sources.append({
                            ...
                            "relevance": round((1 - distance) * 100)  # 转换为0-100的相关度分数
                        })
            ...
            # 构建提示词
            prompt = self._build_qa_prompt(question, contexts, history if use_history else None)
    ...
    def _build_qa_prompt(self, question: str, contexts: List[str], 
    ...
    def _save_qa_history(self, question: str, answer: str, context_ids: List[str]) -> bool:
        """
        保存问答历史
